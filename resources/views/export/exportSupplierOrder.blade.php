...
<div class="container">
    ...
    <div class="info">
        ...
        Người đại diện: {{$supplier->contact_name}}<br>
        Số điện thoại: {{$supplier->phone }}<br>
    ...
    <table>
            ...
            @foreach($details as $i => $item)
                <tr>
                    @if($i == 0)
                        ...
                        <td rowspan="{{ $rowspan }}">{{ \Carbon\Carbon::parse($order->created_at)->format('d/m/Y') }}</td>
        ...
        <tfoot>
        <tr>
            <td colspan="11" style="text-align: right;"><strong>Phát sinh trong kỳ: {{ number_format($total_price, 0, ',', '.') }} VNĐ</strong></td>
        </tr>
        <tr>
            <td colspan="11" style="text-align: right;"><strong>Đã thanh toán trong kỳ: {{ number_format($total_payment, 0, ',', '.') }} VNĐ</strong></td>
        </tr>
        <tr>
            <td colspan="11" style="text-align: right;"><strong>Nợ cuối kỳ (còn lại): {{ number_format($total_price - $total_payment, 0, ',', '.') }} VNĐ</strong></td>
        </tr>
        </tfoot>
    </table>
